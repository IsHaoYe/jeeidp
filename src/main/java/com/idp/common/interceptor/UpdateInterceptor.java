...
public class UpdateInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		try {
			currentUser = (SysUser) ContextHolderUtil.getSession()
					.getAttribute(SessionAttr.USER_LOGIN.getValue());
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i] instanceof BaseEntity) {
					...
					try {
						...
						for (Iterator<String> iterator = baseEntitys.keySet().iterator(); iterator
								.hasNext();) {
							String key = (String) iterator.next();
							List<BaseEntity> baseEntityss = (List<BaseEntity>) baseEntitys.get(key);
							for (Iterator<BaseEntity> iterator2 = baseEntityss.iterator(); iterator2
									.hasNext();) {
