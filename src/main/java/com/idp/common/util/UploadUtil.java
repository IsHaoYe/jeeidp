...
public class UploadUtil {
	...
	public static String getServerUploadBasePath(String subdirectory){
		...
		String path = ResourceUtil.getConfigByName(ResourceEnum.UPLOAD_BASE_PATH.getValue()) + "/" + subdirectory + "/";
			...
			if(fileMap != null && fileMap.size() > 0){
				...
				for(Map.Entry<String, MultipartFile> entity : fileMap.entrySet()){
					...
					if(!mfile.isEmpty()){
						...
						// 文件扩展名验证
						String ext = FileUtil.getExtend(mfile.getOriginalFilename());
						...
						String fileName = sdf1.format(new Date()) + ResourceUtil.getUUID() + "."
							+ FileUtil.getExtend(mfile.getOriginalFilename());
	...
	public static UploadModel uploadFileSealedObject(HttpServletRequest request,UploadModel model) throws Exception {
			...
			if(fileMap != null && fileMap.size() > 0){
				...
				for(Map.Entry<String, MultipartFile> entity : fileMap.entrySet()){
					...
					if(!mfile.isEmpty()){
						...
						ReflectUtil.invokeSetMethod(model.getObject(), entity.getKey(), relativePath + fileName);
						ReflectUtil.invokeSetMethod(model.getObject(), entity.getKey()+"Name", mfile.getOriginalFilename());
					}
				}
			}
		}
		...
		while(enumeration.hasMoreElements()){
		    ...
		    if(StringUtils.isNotEmpty(value)){
		    	...
		    	ReflectUtil.invokeSetMethod(model.getObject(), name, value);
		    }
		}
