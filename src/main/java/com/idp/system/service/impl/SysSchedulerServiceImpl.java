...
import com.idp.system.dao.SysSchedulerDao;
import com.idp.system.entity.SysScheduler;
import com.idp.system.service.SysSchedulerService;
...
public class SysSchedulerServiceImpl implements SysSchedulerService {
	...
	@Override
	public PageModel<SysScheduler> findByPage(SysScheduler sysScheduler,
			PageModel<SysScheduler> page) {
	...
	public void add(SysScheduler sysScheduler) throws Exception {
		...
		SchedulerUtil.addJob(scheduler, sysScheduler.getJobName(), sysScheduler.getJobClass(),
				sysScheduler.getJobGroup(), sysScheduler.getCron());
	...
	public String update(SysScheduler sysScheduler) throws Exception {
		...
		if (SysScheduler.SCHEDULER_ENABLE.equals(oldScheduler.getEnabled())) {
			...
			boolean isRuning = SchedulerUtil.isJobRunning(scheduler, oldScheduler.getJobName(),
					oldScheduler.getJobGroup());

			if (!isRuning) {

				SchedulerUtil.updateJob(scheduler, oldScheduler.getJobName(),
						oldScheduler.getJobGroup(), sysScheduler.getCron());
	...
	public void delete(String ids) throws Exception {
		...
		if (idList != null && idList.size() > 0) {
			...
			for (String id : idList) {
				...
				SchedulerUtil.deleteJob(scheduler, sysScheduler.getJobName(),
						sysScheduler.getJobGroup());
			}
		}
	...
	public String enable(SysScheduler sysScheduler) throws Exception {
		...
		if (SysScheduler.SCHEDULER_ENABLE.equals(sysScheduler.getEnabled())) {
			...
			SchedulerUtil.addJob(scheduler, oldScheduler.getJobName(), oldScheduler.getJobClass(),
					oldScheduler.getJobGroup(), oldScheduler.getCron());
			message = SysMessage.SCHEDULER_ENABLE_OK;
		} else {

			SchedulerUtil.deleteJob(scheduler, oldScheduler.getJobName(),
					oldScheduler.getJobGroup());
